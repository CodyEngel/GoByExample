...
func main() {
	...
	for r := 0; r < 100; r++ {
		go func() {
			total := 0
			for {
				// For each read we pick a key to access, Lock() the mutex to ensure exclusive access to the state, read
				// the value at the chosen key, Unlock() the mutex, and increment the readOps count.
				key := rand.Intn(5)
				mutex.Lock()
				total += state[key]
				mutex.Unlock()
